...
const SignUp = () => {
   ...
   const handleSubmitButton = async () => {
      try {
         const response = await axios.post(`${process.env.REACT_APP_BASEURL}/email/send`, {
            name,
            email,
         });
         const { data } = response;
         ...
         <Grid container spacing={6}>
            <Grid item container justifyContent={"center"} xs={12} md={6}>
               <Box height={"100%"} width={"100%"} maxWidth={{ xs: 500, md: "100%" }}>
                  <img src='images/hero/sponsor.png' height={"100%"} width={"100%"} alt='Sponsor' />
               </Box>
            </Grid>
            <Grid item container alignItems={"center"} justifyContent={"center"} xs={12} md={6}>
               <Box>
                  ...
                  <Typography component='p' color='textSecondary' align={isMd ? "left" : "center"} marginBottom={2}>
                     Interested in sponsoring the Custom Template CDMO build?
                  </Typography>
                  <form noValidate autoComplete='off'>
                     ...
                     {thankYouMessage?.length > 0 && (
                        <Typography component='p' color={lightGreen?.primary?.dark} marginBottom={2}>
                           {thankYouMessage}
                        </Typography>
                     )}
                     <Box
                  ...
                  <Box display='flex' flexWrap='wrap' alignItems='center'>
                     <IconButton component={LinkRouter} to='#' marginRight={1}>
